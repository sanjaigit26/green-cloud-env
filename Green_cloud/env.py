...
class GreenCloudEnv:
    ...
    def step(self, action: Action) -> StepResult:
        ...
        if not job or not region:
            reward = 0.0
        else:
            if action.action_type == "assign" and not job.assigned:
                job.assigned = True
                job.assigned_region = region.name
                reward = calculate_reward(
                    job, region, self.energy_sources, self.time
                )
        ...
        if done:
            # Use grader score as final reward, clamped safely
            raw = self.current_task["grader"](self)
            reward = max(1e-6, min(1.0 - 1e-6, raw))
